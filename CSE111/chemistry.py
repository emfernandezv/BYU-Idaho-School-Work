...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for inner_list in symbol_quantity_list:
        ...
        value = periodic_table_dict[inner_list[SYMBOL_INDEX]]
        mass = value[ATOMIC_MASS_INDEX]
        result = result + (quantity * mass)

    # Return the total molar mass.
    return round(result,5)

def make_known_formulas():
...
def main():
    ...
    list = parse_formula(formula,periodic_table)

    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    molar_mass = compute_molar_mass(list,periodic_table)

    # Compute the number of moles in the sample.
    moles = round(mass / molar_mass,5)
    ...
    # Formula Name
    get_formula_name(formula,make_known_formulas())
